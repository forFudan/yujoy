...
{: .warning }
本方案为基于宇浩拆分的25键纯乱序、拟单编方案，初始学习难度较大，请务必充分了解、分析、平衡其风险和收益后再决定是否学习使用。

## 简介

**卿雲爛兮**是[**宇浩输入法**](http://zhuyuhao.com/yuhao/)的衍生方案，乱序排布、拟单编码。规则共两条：
...
卿雲方案使用强音托，字根读音（小码） 99% 以上情况下都是汉语拼音的第一个字母（有两个规则映射 yu -> v, y -> i），唯一的例外是高频「口」字根。

| 读音（小码） | 规则                                     | 举例                   |
| :----------- | :--------------------------------------- | ---------------------- |
| v            | 所有拼音为 yu，即起始读音为 ㄣ 的，都是v | 雨鱼魚鱼禺予曰月       |
| i            | 所有拼音为 y，即起始读音为 ㄧ 的，都是i  | 一⺁乚乙已乂弋亦衤𧘇言羊 |
| v            | 使用频率最高的字根是v                    | 口                     |

以下为字根图，因为强音托，故而除了以下两类字根外，**不**标注读音（小码）。
...
本方案编码**乱序**，200多个字根的在25键上随机出现，优缺点都十分突出。故总结如下：
...
- 字形相近的部首在键盘上离散。因此，首根编码中可以包含更多信息，从而不需要回取首根小码用以识别，降低规则复杂度。
